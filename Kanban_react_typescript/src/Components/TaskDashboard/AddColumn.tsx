...
interface AddColumnProps {
  ...
  changeState: (e: React.MouseEvent<SVGElement, MouseEvent>) => void;
  submitTitle: (e: ChangeEvent<HTMLInputElement>) => void;
  toggleForm: boolean;
}

export default function AddColumn({ projectId, handleSubmit, changeState, submitTitle, toggleForm }: AddColumnProps) {
  return (
    <>
      {projectId !== 0 ? (
        <th className="bg-light">
          {toggleForm ? (
            <IoIosAdd
              className="mt-1 shadow border rounded custom-hover"
              id="addButton"
        ...
        <Container fluid>
          <h1 className="text-danger">
            404 INVALID REQUEST <IoIosWarning className="text-danger mb-2" />
          </h1>
        </Container>
      )}
    </>
